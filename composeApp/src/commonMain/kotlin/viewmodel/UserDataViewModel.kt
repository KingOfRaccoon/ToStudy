...
class UserDataViewModel(private val onBoardingRepository: OnBoardingRepository) : ViewModel() {
    ...
    fun sendMessage() {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            val messages = messagesFlow.value.filter { it.data?.isUser == true }
            messageText.update { "" }
            ...
            _messagesFlow.update {
                it.filter { it !is Resource.Loading }
                    .plus(obtainRequest(onBoardingRepository.sendMessage(messages.mapNotNull { it.data?.message }
                        .plus(message)
                        .joinToString { it }), it.size
                    )
                    )
            }
        }
    ...
    private fun obtainRequest(resource: Resource<List<String>>, id: Int): Resource<ChatMessage> {
    ...
    fun getModule() = combine(
        ...
        when (modules) {
            ...
            is Resource.Loading -> Resource.Loading()
            is Resource.Success -> Resource.Success(modules.data.find { it.id.numberintrack == current }?.let {
                ModuleWithPages(
                    it.id.numberintrack,
                    it.name,
                    it.quantitypages,
                    ...
                    it.id.idtrack,
                    0,
                    pages.data?.filter { it.idModule == current }.orEmpty()
                        .sortedBy { it.numberInModule }.also {
                            println("pages: $it")
                        }
                )
            })
        ...
        when (tracks) {
            ...
            is Resource.Success -> Resource.Success(tracks.data.find { it.id == current }?.let {
                TrackWithModules(
                    ...
                    modules.data?.filter { it.idTrackId == current }.orEmpty()
                )
            })
        }
    }
