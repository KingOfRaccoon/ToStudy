...
class OnBoardingRepository(private val userDataService: UserDataService) {
    ...
    val onBoardings = arrayOf(
        ...
        OnBoarding(
            listOf(buildAnnotatedString { append("Теперь есть ToStudy") }),
        ...
        OnBoarding(listOf(buildAnnotatedString {
            ...
            append(bullet)
            append("\t\t")
            append("отвечу на все твои вопросы\n")
            append(bullet)
            append("\t\t")
            append("расскажу, как ориентироваться в корпусах\n")
            append(bullet)
            append("\t\t")
            append("познакомлю с культурой колледжа, мероприятиях и студенческих клубах")
        }), "images/onboarding_3.png"),
        OnBoarding(
            listOf(
                buildAnnotatedString {
                    append(
                        "За прохождение этапов адаптации ты будешь получать Хабсы, которые можешь обменять на МЕЕЕЕЕЕРЧ!\n" +
    ...
    val typesUsers = arrayOf(
        TypeUser("Фрешмен", "Нашел себя в списках на зачисление", true),
        TypeUser("Студент", "Я в своем познании настолько преисполнился"),
        TypeUser("Преподаватель", "Мм? Интересно же, что тут у вас"),
    )
